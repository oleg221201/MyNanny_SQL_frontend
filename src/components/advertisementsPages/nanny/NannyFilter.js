...
export const NannyFilter = ({set}) => {
    ...
    const getInfo = useCallback(async () => {
        try {
            await axios.get('/api/filter/nanny').then(result => {
                setInfo(result.data)
            })
        } catch (err) {}
    }, [])
    ...
    const find = async () => {
        ...
        if (data.salaryTo === "") data.salaryTo = null
        if (data.salaryFrom === "") data.salaryFrom = null

        try {
            await axios.post("/api/filter/nanny", {...data}).then(result => {
                set(result.data)
            })
        } catch (err) {}
    }
